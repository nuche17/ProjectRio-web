    ...
    if add_tags_provided:
        ...
        for id in add_tag_ids:
            ...
            if (add_tag.tag_type == "Community" or add_tag.tag_type == "Competition") and not is_user_in_groups(user.id, ['Admin', 'TrustedUser']):
                return abort(420, f'User does not have permission to add tag with ID={id} because of its type')
        ...
        for id in remove_tag_ids:
            ...
            if remove_tag and remove_tag in tag_set.tags:
                if (remove_tag.tag_type == "Community" or remove_tag.tag_type == "Competition") and not is_user_in_groups(user.id, ['Admin', 'TrustedUser']):
                    return abort(420, f'User does not have permission to remove tag with ID={id} because of its type')
...
def get_ladder(in_tag_set=None):
    ...
    game_history_query = (
       ...
       f"WHERE ladder.tag_set_id = {tag_set.id} \n"
        "AND ru.id NOT IN (SELECT user_id FROM user_group_user WHERE user_group_id IN (SELECT id FROM user_group WHERE name_lowercase = 'banned'))\n"
        'GROUP BY \n'
